...
export async function downloadInstagramVideo(igUrl, formatId, savePath) {
  ...
  // Instagram now serves videos in VP9 codec which After Effects CANNOT decode
  // (shows black screen). Use --recode-video to force ffmpeg to re-encode to
  // H.264 (mp4) which AE supports. yt-dlp will skip re-encoding if already H.264.
  await runYtDlp([
    '--no-playlist',
    '--no-warnings',
